# ------------------------------
# Unit.gamma.reg - Versão final (ESTABILIZADA)
# ------------------------------
ugamma.fit <- function(formula = NULL, data = NULL,
  ...
  ## Log-Likelihood Function to be maximized (ESTABILIZADA) ####
  log.ver <- function(theta,Y)
  {
    beta <- theta[1:p]
    phi <- theta[p+1]
    
    # PROTEÇÃO 1: Penaliza phi <= 0 (crucial para log(gamma(phi)))
    if (phi <= 1e-6) return(-1e20)
    
    eta <- X%*%beta
    
    # PROTEÇÃO 2: Limita eta para evitar mi ser 0 ou 1 (over/underflow)
    eta.safe <- pmin(pmax(eta, -30), 30)
    mi <- exp(eta.safe)/(exp(eta.safe)+1)
    
    # PROTEÇÃO 3: Protege o denominador de alpha (1 - mi^(1/phi))
    mi_power <- mi^(1/phi)
    mi_power <- pmin(mi_power, 1 - 1e-6) # Garante que mi_power < 1
    
    alpha <- mi_power / ( 1 - mi_power ) 
    
    # PROTEÇÃO 4: Garante que alpha seja positivo
    if (any(alpha <= 0)) return(-1e20)
    
    l.beta.phi <- sum( phi*log(alpha) - log(gamma(phi)) + (alpha-1)*log(Y) +
                         (phi-1)*log(-log(Y)) )
    
    # PROTEÇÃO 5: Final: retorna valor de penalidade se for NaN/Inf
    if (is.nan(l.beta.phi) | is.infinite(l.beta.phi)) return(-1e20)
    
    return(l.beta.phi)
  }
  
  ## Log-Likelihood Function to be maximized (ESTABILIZADA H0) ####
  log.ver.H0 <- function(theta,Y)
  {
    beta <- theta[1:(p-q)]
    phi <- theta[p-q+1]
    
    # PROTEÇÃO 1: Penaliza phi <= 0
    if (phi <= 1e-6) return(-1e20)
    
    eta <- matrix(X[,-(2:(q+1))],n,p-q)%*%beta
    
    # PROTEÇÃO 2: Limita eta para evitar mi ser 0 ou 1
    eta.safe <- pmin(pmax(eta, -30), 30)
    mi <- exp(eta.safe)/(exp(eta.safe)+1)
    
    # PROTEÇÃO 3: Protege o denominador de alpha
    mi_power <- mi^(1/phi)
    mi_power <- pmin(mi_power, 1 - 1e-6) # Garante que mi_power < 1
  ...
  ## Kickoff for theta (CORRIGIDO) ####
  # logit transform
  z <- log(Y / (1 - Y))
  
  # CORREÇÃO: beta.inic deve vir da regressão linear de Z em X
  beta.inic  <- solve(t(X)%*%X)%*%t(X)%*%z
  
  # calculation of e_chap
  z_fit <- X %*% solve(t(X) %*% X) %*% t(X) %*% z
  e_chap <- z - z_fit
  
  # mu_chap via linear regression in logit
  mu_chap <- exp(z_fit) / (1 + exp(z_fit))
  
  # estimated sigma
  sig_chap <- as.numeric(t(e_chap) %*% e_chap / (n - p))
  
  # phi kickoff - usando a fórmula do usuário
  phi.temp <- (1 / n) * sum((mu_chap * (1 - mu_chap)) / sig_chap - 1)
  
  # CORREÇÃO: Garante phi > 0 (essencial para log(gamma(phi)))
  min_phi_value <- 0.01 
  phi.inic <- max(min_phi_value, phi.temp) 
  ...
  Inv.IF.hat <- try(solve(IF.hat),silent=TRUE)
  Inv.IF.til <- try(solve(IF.til),silent=TRUE)
  Inv.IF.hat.psi.psi <- Inv.IF.hat[2:(q+1),2:(q+1)]          
  Inv.IF.til.psi.psi <- Inv.IF.til[2:(q+1),2:(q+1)]          
  Inv.Inv.IF.hat.psi.psi <- try(solve(Inv.IF.hat.psi.psi),silent=TRUE)  
  ...
  Wald       <- t(psi.hat-psi.0)%*%Inv.Inv.IF.hat.psi.psi%*%t(t(psi.hat-psi.0))
  ...
  for (b in 1:B)
  {
    ...
    # Mantendo o chute de beta original para o bootstrap (apesar de menos ideal)
    ...
    if((!inherits(Estimacao.b, "try-error"))&&(!inherits(Estimacao.b.H0, "try-error")))
    {
      ...
      Inv.IF.hat.b <- try(solve(IF.hat.b),silent=TRUE)
      Inv.IF.til.b <- try(solve(IF.til.b),silent=TRUE)
      Inv.IF.hat.psi.psi.b     <- Inv.IF.hat.b[2:(q+1),2:(q+1)]
      Inv.IF.til.psi.psi.b     <- Inv.IF.til.b[2:(q+1),2:(q+1)]
      Inv.Inv.IF.hat.psi.psi.b <- try(solve(Inv.IF.hat.psi.psi.b),silent=TRUE)
...
#dat= read_excel("Scripts e doc TCC/TCC.xlsx")
#Tcc=read_excel("Scripts e doc TCC/TCC.xlsx")
#dat=dat[,c(-1)]
#colnames(dat)=c("Y","X1","X2","X3","X4","X5","X6")
#summary(dat)
#
