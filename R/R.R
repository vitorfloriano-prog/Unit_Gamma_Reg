# ------------------------------
# Unit.gamma.reg - Final Version - CORRECTED FOR PORTABILITY
# ------------------------------
ugamma.fit <- function(formula = NULL, data = NULL,
  ...
  for (b in 1:B)
  {
    ...
    # CORRECTION 1: Ensures that the IF statement works with IF statements, preventing the 'length > 1' error.
    if(all((class(Estimacao.b) != "try-error") & (class(Estimacao.b.H0) != "try-error")))
    {
      ...
      # # CORRECTION 2: Ensures that the IF statement works with IF statements, preventing the 'length > 1' error.
...
summary.Unit.gamma <- function(object, digits = 3, ...) {
  ...
  cat("\nAIC.Gamma:", round(object$AIC.Gama, 4), " BIC.Gamma:", round(object$BIC.Gama, 4), "\n")
